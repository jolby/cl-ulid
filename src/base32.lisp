...
(defun make-timestamp-bytes-array ()
  (make-array +timestamp-bytes-len+ :element-type '(unsigned-byte 8)))

(defun make-timestamp-string-array ()
  (make-array +timestamp-string-len+ :element-type 'character))

(defun make-randomness-bytes-array ()
  (make-array +randomness-bytes-len+ :element-type '(unsigned-byte 8)))

(defun make-randomness-string-array ()
  (make-array +randomness-string-len+ :element-type 'character))

(defun make-ulid-bytes-array ()
  (make-array +ulid-bytes-len+ :element-type '(unsigned-byte 8)))

(defun make-ulid-string-array ()
  (make-array +ulid-string-len+ :element-type 'character))
...
(defun decode-ulid-to-bytes (encoded)
